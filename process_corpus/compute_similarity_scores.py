...
# Function to generate a CSV with the 30 most similar poems and their similarity scores
def generate_top_k_similarity_csv(similarity_matrix, poem_files, output_csv='similarity_matrix.csv', top_k=30):
    ...
    with open(output_csv, mode='w', newline='', encoding='utf-8') as csv_file:
        ...
        # Write the header (Poem and the top 30 most similar poem names and their similarity scores)
        header = ['poem'] + [f'top_{i+1}_similar_poem' for i in range(top_k)] + [f'top_{i+1}_similarity_score' for i in range(top_k)]
        ...
        for i, poem_name in tqdm(enumerate(poem_names), total=len(poem_names), desc="Sorting and Saving Top Similarities"):
            ...
            top_k_similarity_scores = [row[idx] for idx in top_k_poem_indices]  # Get the corresponding similarity scores

            # Write the row to the CSV (first column is the poem itself, followed by top_k most similar poems and their scores)
            writer.writerow([poem_name] + top_k_poem_names + top_k_similarity_scores)


def main():
    # Define input and output paths
    input_folder = 'corpus_pt'  # Folder with embeddings
    output_csv = 'similarity_matrix_scores.csv'  # Output CSV for similarity
    ...
    # Generate the CSV file with ordered similarities and their scores
